...
import { TeamBasicDetail } from "@/services/team/team.type";
...
export default function LeaderboardScreen() {
  ...
  const [results, setResult] = useState<
    GlobalRankDetail[] | ActivityRankDetail[]
  >([]);
  const [myTeamBest, setMyTeamBest] = useState<
  ...
  const fetchMyTeamLatestResult = async (rankings: ActivityRankDetail[]) => {
    ...
    const latestAttempt = rankings
      .filter((item) => item.teamId === team.id)
      .reduce(
        (latest: ActivityRankDetail | null, current: ActivityRankDetail) => {
  ...
  return (
    ...
    >
      ...
      <View style={styles.podiumContainer}>
        {top3 && (
          <PodiumCard
            ...
            attemptNo={
              results[2] && "attemptNo" in results[2]
                ? results[2].attemptNo.toString()
                : undefined
            }
          />
        )}
        ...
        {top1 && (
          <View style={{ marginBottom: 30 }}>
            <PodiumCard
              ...
              attemptNo={
                results[0] && "attemptNo" in results[0]
                  ? results[0].attemptNo.toString()
                  : undefined
              }
            />
          </View>
        ...
        {top2 && (
          <PodiumCard
            ...
            attemptNo={
              results[1] && "attemptNo" in results[1]
                ? results[1].attemptNo.toString()
                : undefined
            }
          />
        )}
      ...
      <View style={styles.rankingList}>
        {results.slice(3).map((item, i) => (
          <RankingCard
            ...
            attemptNo={
              item && "attemptNo" in item
                ? item.attemptNo.toString()
                : undefined
            }
          />
        ))}
      ...
      <View style={{ marginTop: 10 }}>
        <Text style={styles.section}>My Team Best:</Text>
        ...
        <RankingCard
          ...
          attemptNo={
            myTeamBest && "attemptNo" in myTeamBest
              ? myTeamBest.attemptNo.toString()
              : undefined
          }
        />
      </View>
      ...
      {dropdownValue !== "global" && (
        <View>
          <Text style={styles.section}>My Team Latest Attempt:</Text>

          <RankingCard
            rank={myTeamLatest?.rank?.toString() || "-"}
            score={
              myTeamLatest ? calculateDisplayScore(myTeamLatest.score) : "-"
            }
            teamName={team?.name || "-"}
            imageUrl={team?.logo || defaultLogo}
