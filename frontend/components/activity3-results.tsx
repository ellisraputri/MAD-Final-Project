    ...
    return(
        ...
        <View style={resultStyles.subsContainer}>
            ...
            <Accordion title="Calculation" marginBottom={0}>
                <Text style={resultStyles.descText}>The object is thin paper. So, </Text>
                <Equation latex='k = 0.05 \\text{ } N/rad' fontSize={13}/>

                <Text style={[resultStyles.descText, {marginTop: 15, marginBottom: 2}]}>The observed bend angle:</Text>
                <Equation latex={`\\\\theta = ${props.bendCalculated.toFixed(3)}^{\\\\circ} \\\\approx ${bendRadian.toFixed(3)} rad`} fontSize={13}/>
                
                <Text style={[resultStyles.descText, {marginTop: 15, marginBottom: 2}]}>The applied force:</Text>
                <Equation latex={`F \\\\approx k \\\\cdot \\\\theta`} fontSize={13}/>
                <Equation latex={`F \\\\approx ${0.05} \\\\cdot ${bendRadian.toFixed(3)} \\\\approx ${force.toFixed(3)} N`} fontSize={13}/>
            </Accordion>
        </View>
        </View>
    )
    ...
    <>
      <ScrollView style={styles.container} contentContainerStyle={{ paddingTop: 20, paddingBottom: 100 }}>
        ...
        <Section title="Theory">
          ...
          <Text style={styles.paragraph}>Moving air applies force to objects. Paper bends due to flexibility (plasticity), and repeated bending can weaken it.</Text>

          <Text style={styles.subtitle}>Forces to Bend Paper</Text>
          <Text style={[styles.paragraph, {marginBottom: 2}]}>For bending a sheet of paper:</Text>
          <Equation latex={"F \\\\approx k \\\\cdot \\\\theta"} fontSize={13} />

          <Text style={[styles.paragraph, {marginBottom: 2}]}>where:</Text>
          <Equation latex={"F = \\\\text{force applied } (N)"} fontSize={13} />
          <Equation latex={"\\\\theta = \\\\text{bend angle } (radians)"} fontSize={13} />
          <Equation latex={"k = \\\\text{stiffness coefficient }"} fontSize={13} />

          <Text style={[styles.paragraph, {marginTop: 10, marginBottom: 2}]}>In reality, k is related to flexural rigidity</Text>
          <Equation latex={"D = \\\\frac{Et^3}{12(1-v^2)}"} fontSize={13} />
          <Text style={[styles.paragraph, {marginTop: 0}]}>but for classroom experiments we can use approximate empirical values. The force required increases strongly with stiffness.</Text>

          <Text style={styles.subtitle}>Approximate k Values for Different Paper</Text>
          <Table
              columns={[
          ...
          />
          <Text style={styles.paragraph}>These are rough classroom values to let students estimate forces. The point is to see relative differences between materials.</Text>
        </Section>
