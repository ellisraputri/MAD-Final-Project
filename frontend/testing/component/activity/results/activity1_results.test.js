...
jest.mock(
  "@/components/activity/results/activity-result-base",
  () => {
    const React = require("react");
    const { View, Text } =
      require("react-native");
...
jest.mock(
  "@/components/activity/results/useActivityResults",
  () => ({
    __esModule: true,
    default: (...args) =>
      mockUseActivityResult(...args),
  })
);
...
describe(
  ...
  () => {
    ...
    it("renders no-bounce calculation when selected", () => {
      ...
      fireEvent.press(
        getByText("Select No Bounce")
      );

      expect(
        getByText(
          /Object goes from impact speed downward/i
        )
      ).toBeTruthy();
    });
  }
