...
import ActivityOneSubmissionCard from "@/components/activity/submission/activity1-submission-card"; // Adjust path if necessary
...
describe("ActivityOneSubmissionCard", () => {
  const mockOnChangeMass = jest.fn();
  ...
  it("renders correctly with all props and a video URI", () => {
    const { getByText, getByDisplayValue } = render(
      <ActivityOneSubmissionCard {...defaultProps} />
    );
    ...
    expect(getByText("Mass of toy (gram)")).toBeTruthy();
    expect(getByText("Prediction")).toBeTruthy();
    expect(getByText("Time to hit ground (seconds)")).toBeTruthy();
  ...
  it("renders 'No video' text when videoUri is null", () => {
    const { getByText, queryByText } = render(
      <ActivityOneSubmissionCard {...defaultProps} videoUri={null} />
    );
  ...
  it("triggers onChangeMass and onChangeTime when text inputs change", () => {
    const { getByDisplayValue } = render(
      <ActivityOneSubmissionCard {...defaultProps} />
    );

    // Find the inputs by their displayed value
    const massInput = getByDisplayValue("50");
    const timeInput = getByDisplayValue("2.5");

    // Simulate typing
    fireEvent.changeText(massInput, "100");
    fireEvent.changeText(timeInput, "5.0");

    expect(mockOnChangeMass).toHaveBeenCalledWith("100");
    expect(mockOnChangeTime).toHaveBeenCalledWith("5.0");
  });
